...
async function testSbDynamic() {
  const form = new URLSearchParams({ eventId: '35938017', eventType: '4', apiSiteType: '2', isDynamicUpdate: '1' });
  ...
  for (const m of sbMarkets) {
    if (m.marketStatus === 1 || m.apiSiteStatus === 'ACTIVE') {
      console.log('Active Market JSON:', JSON.stringify(m, null, 2));
      break;
    }
  }
}

testSbDynamic().catch(console.error);
