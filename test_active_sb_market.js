...
async function testActiveSbMarket() {
  ...
  for (const m of sbMarkets) {
    if (m.marketName && m.marketName.includes('1st innings overs 0 to 12')) {
      console.log('Target Market Found:', JSON.stringify(m, null, 2));
    }
  }
}

testActiveSbMarket().catch(console.error);
