...
async function testParseBookmode() {
  ...
  console.log(`Found ${activeList.length} Active Sportsbook Markets:\n`);
  ...
  activeList.forEach((m, idx) => {
    ...
    console.log(`${idx + 1}. Market: "${m.marketName}"`);
    if (specifier.total) console.log(`   Specifier Total: ${specifier.total}`);
    console.log(`   Parsed Odds Array (bookMode):`, oddsArray);
  });
}

testParseBookmode().catch(console.error);
